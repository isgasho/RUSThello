...
impl AiPlayer {
            ...
            _num_moves => {
                ...
                while depth <= 5 {
                    ...
                    for &mv in moves.iter() {
                        ...
                        let score =
                            AiPlayer::ai_eval_iddfs(&turn_after_move, depth)
                            .expect("Something went wrong with `AiPlayer::ai_eval_iddfs`!");
    ...
    fn ai_eval_iddfs(turn: &turn::Turn, depth: u32) -> Result<Score> {
        if turn.get_state().is_none() {
            ...
            let mut score = try!(AiPlayer::ai_eval_with_leftover_iddfs(turn, depth));
            // Add some randomness
    ...
    fn ai_eval_with_leftover_iddfs(turn: &turn::Turn, depth: u32) -> Result<Score> {
        ...
        while let Some(coord) = moves.pop() {
            ...
            scores.push(match turn_after_move.get_state() {
                _ => {
                    let new_score = try!(AiPlayer::ai_eval_with_leftover_iddfs(&turn_after_move, depth - 1));
                    new_score
                }
            });
        }
